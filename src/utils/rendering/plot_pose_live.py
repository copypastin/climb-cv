...
def plot_world_landmarks(ax, landmarks, visibility_th=0.5) -> None:
    
    if not landmarks:
        return
    
    landmarks_iter = landmarks.landmark if hasattr(landmarks, "landmark") else landmarks


    landmark_point = []

    for index, landmark in enumerate(landmarks_iter):
